...
# Formats Stage 2 can write. `laz` is the default because a binary PLY is a raw memory
# dump -- measured 37.00 bytes/point, 29.2 GB for the 14 plots against 4.4 GB of source
# -- and the clouds have to cross a network to reach a rented GPU. ForAINet itself only
# reads PLY, so a LAZ export is converted back inside the container by pipeline/convert.py.
_OUTPUT_FORMATS = ("laz", "las", "ply")
...
def _preprocess_one(
    ...
    """Center one cloud, write ``<stem>.<output_format>`` + ``<plot>_offsets.yml``.

    ``split`` (``train``/``val``/``test``, or ``None``) only decides the output file's
    ...
    ``output_format`` is ``laz`` (default), ``las`` or ``ply``. The point data is
    identical either way -- only the container differs. ForAINet reads PLY, so a LAZ
    export is converted back by ``pipeline/convert.py``; see the module docstring.
    ...
    # The coordinates are already centered, so the LAS header offset ends up at 0 and a
    # reader gets the centered values straight back -- the conversion inside the training
    # container needs no offsets file, only this cloud.
    stem = f"{plot}_{split}" if split else plot
    out_path = out_dir / f"{stem}.{output_format}"
    if output_format == "ply":
        _write_ply(array, out_path)
    else:
        _write_las(array, out_path, meta)
    ...
    offsets = {
        ...
        # Kept alongside `output_file` so offsets files written before Stage 2 could
        # emit LAZ still read the same way. Drop it once nothing consults it.
    ...
    log.info(
        ...
        "yes" if meta["crs_wkt"] else "none", out_path.name, offsets_path.name,
    )
    return n_points
...
def prep_forainet(
    ...
    output_dir: str = r"D:\Podyplomowe\04_AI_Intro\assignment\SegmentedForests\ForAINet_export",
    output_format: str = "laz",
    semantic_field: Optional[str] = None,
    tree_id_field: Optional[str] = None,
    intensity_field: Optional[str] = None,
    ...
    output_dir:
        (preprocess) Folder receiving ``<plot>.<output_format>`` +
        ``<plot>_offsets.yml``; created if missing. Also the default place restore mode
        looks for the offset files.
    output_format:
        (preprocess) ``"laz"`` (default), ``"las"`` or ``"ply"`` — the container the
        point data is written in; the points themselves are identical either way.
        **LAZ is the default because a binary PLY is an uncompressed memory dump**
        (measured 37.00 bytes/point; 29.2 GB for the 14 plots against 4.4 GB of source),
        which matters when the clouds have to be uploaded to a rented GPU. ForAINet
        reads PLY only, so a LAZ export is expanded again by ``pipeline/convert.py``,
        cheaply, inside the training container. Use ``"ply"`` when training locally and
        point ``output_dir`` at ``paths.forainet_raw``.
    ...
    if output_format not in _OUTPUT_FORMATS:
        raise ValueError(
            f"output_format must be one of {list(_OUTPUT_FORMATS)}, got {output_format!r}")
    ...
    if mode == "preprocess":
        ...
        # ForAINet globs `raw/**/*.ply`, so anything but PLY written into its data tree
        # is invisible to it -- training would start on an empty dataset. Cheap to check,
        # and the failure it prevents looks like a framework bug rather than a typo.
        if output_format != "ply" and "raw" in {p.name for p in out_dir.parents}:
            log.warning(
                "output_format=%s but output_dir (%s) sits inside a ForAINet raw/ tree, "
                "which is globbed for *.ply only — ForAINet will not see these files. "
                "Export elsewhere and convert with pipeline/convert.py, or set "
                "output_format=ply.", output_format, out_dir)
        ...
        for src_path in src_files:
            ...
            stem = f"{plot}_{which}" if which else plot
            out_cloud = out_dir / f"{stem}.{output_format}"
            if not overwrite and out_cloud.exists():
                log.info("[%s] %s already exists and overwrite=False — skipping", plot, out_cloud.name)
                skipped.append(plot)
                continue
            if dry_run:
                log.info("[%s] DRY RUN: would center %s -> %s + %s_offsets.yml in %s",
                         plot, src_path.name, out_cloud.name, plot, out_dir)
            ...
            # changes) or change format, leaving the previous file behind. ForAINet
            # globs raw/**/*.ply, so a leftover would load this plot TWICE, in two
            # different splits -- training on its own test data without complaint.
            # Every split AND every format is swept, so switching output_format does
            # not strand a .ply beside the new .laz either.
            stems = [plot] + [f"{plot}_{s}" for s in _SPLITS]   # incl. the un-split name
            for other_stem in stems:
                for ext in _OUTPUT_FORMATS:
                    stale = out_dir / f"{other_stem}.{ext}"
                    if stale == out_cloud or not stale.exists():
                        continue
                    stale.unlink()
                    log.warning("[%s] removed %s — this plot is now %s, and leaving both "
                                "would put it in two splits at once",
                                plot, stale.name, out_cloud.name)
